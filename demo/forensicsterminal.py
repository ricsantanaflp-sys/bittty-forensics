...
class StdoutFrontend:
    ...
    def __init__(self):
        ...
        # Atributos para captura de comandos e saídas JSON
        self.current_cmd_buffer = ""
        self.last_command = ""
        self.last_output_raw = ""
    ...
    def handle_pty_data(self, data: str):
        ...
        """
        try:
            # Acumula a saída do processo no buffer do último comando
            self.last_output_raw += data
    ...
    def save_evidence_json(self):
        """Salva o último comando executado e sua saída em um arquivo JSON."""
        if not self.last_command:
            self.status_msg = "Nenhum comando registrado para salvar!"
            self.render_screen()
            return

        output = self.clean_output(self.last_output_raw)
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

        # Cálculo do SHA256 (concatenação do comando com a saída)
        hash_content = (self.last_command + output).encode("utf-8")
        hash_sha256 = hashlib.sha256(hash_content).hexdigest()
        ...
        filepath = Path(self.json_filename)
        data_list = []
        ...
        logger.info("Evidência salva em %s: %s", filepath.resolve(), entry)
        self.status_msg = f"SALVO em '{self.json_filename}' ([{self.last_command}])"
        self.render_screen()

    def handle_input(self, data: str):
        """Forward input to bittty, intercepting Ctrl+S and host SGR mouse reports."""
        # Intercepta a tecla atalho Ctrl+S (\x13)
        if "\x13" in data:
            data = data.replace("\x13", "")
            self.save_evidence_json()

        # Monitora caracteres digitados para registrar o último comando
        for char in data:
            if char in ("\r", "\n"):
                if self.current_cmd_buffer.strip():
                    self.last_command = self.current_cmd_buffer.strip()
                self.current_cmd_buffer = ""
                self.last_output_raw = ""
                self.status_msg = ""
            elif char in ("\b", "\x7f"):  # Backspace
                self.current_cmd_buffer = self.current_cmd_buffer[:-1]
            elif ord(char) >= 32 and ord(char) != 127:  # Caracteres normais
